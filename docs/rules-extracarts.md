...
| `tickets-via-bin` — create/close/status through `bin/` | intake, close | (TICKETS.md) | `pre-bash` regex; allows comments, body edits, `sub_issues`, `blocked_by` | hook only; `bin/check-tickets` is the verifier |
...
183 cases in the engine suite and 11 in the overlay suite. All pass.
