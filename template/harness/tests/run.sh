...
expect deny:git-no-bypass "$PB" "$(bash_json "git -C $P commit -m 'x' -n")" "bypass: -n after a quoted message"
expect deny:git-no-bypass "$PB" "$(bash_json "git -C $P commit -m \"x\" -n")" "bypass: -n after a double-quoted message"
expect allow "$PB" "$(bash_json "git -C $P commit -m 'handle -n flag'")" "positive: -n inside the message"
...
expect allow "$PB" "$(bash_json "gh issue transfer 12 acme/frontend")" "positive: transfers are allowed"
